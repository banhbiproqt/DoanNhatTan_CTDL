...
class LinkedList:      # tạo class LinkedList
    def __init__(self): # phương thức khởi tạo của lớp LinkedList
        ...
        self._tail = None #  tail để tham chiều của nút cuối cùng
    ...
    def reversed(self):
        current_node = self._head#  Gán nút đầu của danh sách liên kết cho biến `current_node`.
        previous_node = None#  Đặt biến `previous_node` thành `None` ban đầu.
        next_node = None#   Đặt biến `next_node` thành `None` ban đầu.
        while current_node is not None:# Lặp lại cho đến khi `current_node` đến cuối danh sách.
            next_node = current_node._next#  Lưu nút tiếp theo trong danh sách (được trỏ đến bởi `current_node._next`) trong biến `next_node`.
            current_node._next = previous_node#Dòng 6: Thay đổi con trỏ `_next` của `current_node` để trỏ đến `previous_node`.
            previous_node = current_node # Gán nút hiện tại cho `previous_node` cho vòng lặp tiếp theo
            current_node = next_node#  Di chuyển `current_node` đến nút tiếp theo trong danh sách

        self._head = previous_node#Cập nhật thuộc tính `_head` của danh sách liên kết để trỏ đến nút cuối cùng (mà hiện là nút đầu tiên sau khi đảo ngược).
