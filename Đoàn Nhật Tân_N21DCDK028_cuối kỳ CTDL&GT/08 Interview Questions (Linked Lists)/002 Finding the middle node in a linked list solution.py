...
class LinkedList:      # tạo class LinkedList
    ...
    def find_middle_node(linked_list):  # hàm tìm kiếm nút giữa của linked list
        if linked_list.isempty(): # kiểm tra nếu linked list rỗng thì thoát
            return None

        slow = linked_list._head #  khởi tạo 2 con trỏ là slow và fast trỏ đến phần từ đầu tiên của linked list
        fast = linked_list._head

        while fast._next and fast._next._next: # thực hiện vòng lặp cho đến còn trỏ tới một nút
            slow = slow._next #   slow tiến tới một bước thì fast tiến tới 2 bước => vì vậy khi fast tiến tới cuối linked list thì fast tiến tới nút giữa của linked list
            fast = fast._next._next#

        return slow._element # trả về giá trị của nút giữa
...
print(' middle node is ' , L.find_middle_node())